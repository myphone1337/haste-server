...
DocumentHandler.prototype.handleHead = function(request, response) {
  ...
  this.store.getMetadata([key], function(reply) {
    ...
    if (metadata !== null && Date.now() > metadata.expire) {
...
DocumentHandler.prototype.handleDelete = function(request, response) {
  ...
  this.store.delete([key], function(reply) {
    response.writeHead(200, reply);
    response.end();
  });
};
